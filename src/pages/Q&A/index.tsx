...
const QA: React.FC = () => {
  ...
  const [isadd, setIsadd] = useState('5');

  const onSearch = () => {
    message.success('已找到相关问答');
    setIsshow(true);
  };
  ...
  return (
    <PageContainer ghost>
      <Search key="4" placeholder="ROS" enterButton onSearch={onSearch} />
      {isshow ? (
        <>
          <MyProList
      ...
      <TextArea
        ...
        style={{ marginTop: '12px' }}
      />
      <Button style={{ float: 'right', marginTop: '2px' }} onClick={addContent}>
        发布
      </Button>
    </PageContainer>
  );
